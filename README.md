...
| ✍️ 批量生成字幕 | 给缺字幕的视频批量生成字幕（识别 + 翻译），生成后可自动去重、拆长句。**需自行准备第三方识别引擎**，见[说明](docs/第三方工具集成.md) |
| 🔓 马赛克破解 | 批量调用 Lada 破解有码视频，支持并发、显存门控、产出时长校验。**需自行准备 Lada**，见[说明](docs/第三方工具集成.md) |
| 🔄 破解视频替换 | 把 `xxx.restored.mp4` 替换回库中对应位置（「马赛克破解」的产物可直接在这里接手） |
...
这两项功能**依赖第三方的命令行程序**，本仓库**不包含**它们（体积与许可都不合适）：

| 功能 | 需要准备 | 下载 |
| --- | --- | --- |
| 批量生成字幕 | `infer.exe`（Faster-Whisper / TransWithAI 打包版） | [TransWithAI/Faster-Whisper-TransWithAI-ChickenRice](https://github.com/TransWithAI/Faster-Whisper-TransWithAI-ChickenRice) |
| 马赛克破解 | `lada-cli.exe`（Lada） | [codeberg.org/ladaapp/lada/releases](https://codeberg.org/ladaapp/lada/releases) |

解压到任意位置，然后在「设置 → 第三方工具」填路径（**留空会自动查找**）即可。
参数不是写死的：程序会读工具的 `--help` / `--list-*` 输出，
**按你那个版本真正支持的选项**发命令，并且把可用的预设/模型/设备直接填进下拉框。

完整的下载、前置条件、配置与排错说明见 **[docs/第三方工具集成.md](docs/第三方工具集成.md)**。
