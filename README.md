...
这两个功能**依赖第三方的命令行程序**，本仓库**不包含**它们（体积与许可都不合适）：
...
参数不是写死的：程序会读工具的 `--help` / `--list-*` 输出，**按你那个版本真正支持的选项**发命令，
并把可用的预设/模型/设备直接填进下拉框。完整的下载、前置、配置与排错说明见
**[docs/第三方工具集成.md](docs/第三方工具集成.md)**。
